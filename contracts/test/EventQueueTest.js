...
// change CAPACITY in EventQueue.sol to 3 before running tests
...
contract('EventQueue', async (accounts) => {
    ...
    it("should enqueue 3 out of 4 items for topic 1", async () => {
        ...
        let countBefore = await queue.getCount("topic1");
        ...
        let countAfter = await queue.getCount("topic1");
    ...
    it("should enqueue 1 items for topic 2", async () => {
        ...
        let countBefore = await queue.getCount("topic2");
        assert.equal(countBefore.valueOf(), 0);

        await queue.enqueue("topic2", "aaa");

        let countAfter = await queue.getCount("topic2");
    ...
    it("should get 1st queued item for topic 1", async () => {
        ...
        let result = await queue.peek("topic1", 0);
    ...
    it("should get 1st queued item for topic 2", async () => {
        ...
        let result = await queue.peek("topic2", 0);
        let address = result[0];
        let item = result[1];
        assert.equal(address, accounts[0]);
    ...
    it("should get nothing for topic 2", async () => {
        ...
        let countAfter = await queue.getCount("topic3");
        assert.equal(countAfter.valueOf(), 0);
    });
    ...
    it("should get nothing for topic 3", async () => {
        ...
        let countBefore = await queue.getCount("topic3");
        assert.equal(countBefore.valueOf(), 0);

        let result = await queue.peek("topic3", 0);
        let address = result[0];
        let item = result[1];
        assert.equal(address, 0x0);
        assert.equal(item, "");
    ...
    it("should get capacity", async () => {
        let queue = await myContract.deployed();
        let capacity = await queue.getCapacity("topic1");
